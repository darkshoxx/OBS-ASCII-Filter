...
pixel_data vertex_shader_a_2(vertex_data vertex)
{
    pixel_data pixel;
...
float4 pixel_shader_a_2(pixel_data pixel) : TARGET
{
    float2 position = pixel.uv * float2(width, height);
    int column = ceil(position.x/((float)width/cells_h));
    int row = ceil(position.y/((float)height/cells_v));
    if ((column + row) % 2 == 0)
    {
        return black;
    }
...
technique Draw
{
    pass
    {
        vertex_shader = vertex_shader_a_2(vertex);
        pixel_shader  = pixel_shader_a_2(pixel);
    }
}